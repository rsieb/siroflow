...
while true == true
  ...
  @keuze.each_index do |number|
    ...
    print "[#{(number+1).to_s(36)}] #{@keuze[number].gsub(".routine2.yaml","")}  "
  end
  print " or e[X]uit"
  ...
  @tasks.each_index do |nummer|
    ...
    sleutel = @tasks[nummer].keys[0]
    puts "Sleutel is #{sleutel}"

    # create the array of net duration values
    records = @tasks[nummer][sleutel]
    waarden = Array.new()
    records.each_index do |recordnummer|
      waarden.push( @tasks[nummer][sleutel][recordnummer]['netdur'])
    end

    # FIXED 2010-09-21_1128-0700  replace single values in YAML with hash structure containing dates and status and notes, use OmniOutliner to model first
  ...
  @myevent = app("iCal").make(:at => app.calendars[its.name.eq("Tracker")].calendars[1].events.end, :new => :event, :with_properties => {
    ...
    :summary => "#{@laadbestand.gsub(".routine2.yaml","")}"
    })
    app("iCal").activate
    @myevent.show
    @geschoren = 0
